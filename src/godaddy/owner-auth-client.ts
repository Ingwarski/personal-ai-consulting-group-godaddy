...
export const ownerAuthClientJavaScript = String.raw`(() => {
  ...
  const localDestinations = new Set(["/auth/sign-in", "/settings", "/operations/runtime", "/consultation", "/instructions"]);
  ...
  document.addEventListener("submit", async (event) => {
    ...
    // Named controls can shadow form.action; always read the attribute.
...
// Bind HTML to its exact handler bytes so cached action code cannot control a
// newer form contract.
