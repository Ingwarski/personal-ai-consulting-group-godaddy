...
export function matrixSetupDocument(view: MatrixSetupView, token: string): string {
  ...
  ${mysqlTransport === undefined ? "" : `<section><h2>Транспорт MySQL</h2><ul><li>Node підключається до бази: ${yes(mysqlTransport.nodeDatabaseReachable)}</li><li>Поточне з’єднання Node зашифроване: ${mysqlTransport.nodeSessionEncrypted === "unknown" ? "невідомо" : yes(mysqlTransport.nodeSessionEncrypted)}</li><li>Додатковий CA для Node налаштований: ${yes(mysqlTransport.nodeExtraCaConfigured)}</li><li>Системний CA для Node запитаний: ${yes(mysqlTransport.nodeSystemCaRequested)}</li><li>TLS на сервері MySQL: ${mysqlTransport.serverTlsSupport === "available" ? "доступний" : mysqlTransport.serverTlsSupport === "disabled" ? "вимкнений" : "невідомо"}</li><li>Сервер вимагає захищений транспорт: ${mysqlTransport.secureTransportRequired === "unknown" ? "невідомо" : yes(mysqlTransport.secureTransportRequired)}</li><li>Окреме підключення з перевіркою сертифіката: <code>${escape(mysqlTransport.verifiedTlsConnection)}</code></li></ul><p>Показано лише нормалізовані ознаки; адреса, обліковий запис, схема, сертифікат і помилки не виводяться.</p></section>`}
