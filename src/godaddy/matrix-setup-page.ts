...
export function matrixSetupDocument(view: MatrixSetupView, token: string): string {
  ...
  const stateLabels: Record<string, string> = {
    ...
    prepared: mysql ? "Програми перевірено. Підключіть наявне сховище MySQL або явно підготуйте новий окремий пристрій." : "Обидві програми перевірено. Приватні каталоги підготовлено; перевірка HTTP-доступу пройдена.",
  ...
  ${mysqlTransport === undefined ? "" : `<section><h2>Транспорт MySQL</h2><ul><li>Node підключається до бази: ${yes(mysqlTransport.nodeDatabaseReachable)}</li><li>Поточне з’єднання Node зашифроване: ${mysqlTransport.nodeSessionEncrypted === "unknown" ? "невідомо" : yes(mysqlTransport.nodeSessionEncrypted)}</li><li>Додатковий CA для Node активний: ${yes(mysqlTransport.nodeExtraCaConfigured)}</li><li>Додатковий системний CA для Node активний: ${yes(mysqlTransport.nodeAdditionalSystemCaActive)}</li><li>TLS на сервері MySQL: ${mysqlTransport.serverTlsSupport === "available" ? "доступний" : mysqlTransport.serverTlsSupport === "disabled" ? "вимкнений" : "невідомо"}</li><li>Сервер вимагає захищений транспорт: ${mysqlTransport.secureTransportRequired === "unknown" ? "невідомо" : yes(mysqlTransport.secureTransportRequired)}</li><li>Перевірка ланцюжка сертифіката: <code>${escape(mysqlTransport.verifiedTlsConnection)}</code></li><li>Перевірка ланцюжка та імені сервера: <code>${escape(mysqlTransport.verifiedTlsIdentityConnection)}</code></li></ul><p>Показано лише нормалізовані ознаки; адреса, обліковий запис, схема, сертифікат і помилки не виводяться.</p></section>`}
  ...
  ${view.state === "prepared" ? (mysql ? `<p>Новий пристрій має бути окремою сесією бота без попередніх ключів шифрування. Наявні дані не видаляються; зайняте сховище не перезаписується.</p>${form("resume", "Підключити наявний пристрій через MySQL")}${form("start_fresh", "Підготувати новий окремий пристрій у MySQL")}` : `<p>Новий пристрій має бути окремою сесією бота без попередніх ключів шифрування. Порожнє сховище для наявного пристрою не допускається.</p>${form("start_fresh", "2. Підготувати новий окремий пристрій бота")}${form("resume", "Продовжити з наявним сховищем цього пристрою")}`) : ""}
