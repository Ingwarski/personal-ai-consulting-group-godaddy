...
export function matrixPreviewVerifierResponse(request: Request): Response | undefined {
  ...
  return new Response(`<!doctype html><html lang="uk"><head><meta charset="utf-8"><title>Перевірка приватності Preview</title><script src="${MATRIX_VERIFIER_SCRIPT_PATH}" defer></script></head><body><h1>Перевірка приватності Preview</h1><p data-verifier-status>Очікуємо захищений запит із Published.</p><p>Ця сторінка лише читає тимчасові контрольні файли. Вона не змінює Preview і не передає cookies, ключі або вміст відповідей.</p></body></html>`,
