...
export function parseGodaddyDatabaseConfiguration(
  ...
  const connectionLimitValue = environment.DB_POOL_LIMIT;
  const port = portValue === undefined ? undefined : Number.parseInt(portValue, 10);
  const connectionLimit = connectionLimitValue === undefined ? 8 :
    typeof connectionLimitValue === "string" && /^(?:[4-9]|1[0-9]|2[0-9]|3[0-2])$/u.test(connectionLimitValue)
      ? Number.parseInt(connectionLimitValue, 10) : undefined;
  if (
  ...
  return { ok: true, value: Object.freeze({ host, port, database, user, password, connectionLimit }) };
}
