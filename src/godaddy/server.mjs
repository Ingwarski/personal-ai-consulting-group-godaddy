...
import { GoDaddyMatrixCapabilityProbe } from "./capability-probe.mjs";
...
export function createGodaddyServer({
  ...
  capabilityProbe = new GoDaddyMatrixCapabilityProbe()
} = {}) {
  ...
  return createServer(async (request, response) => {
    ...
    if (url.pathname === "/__godaddy-capability-probe") {
      if (!status.ok) return json(response, 503, { status: "blocked", code: status.code });
      if (request.method === "POST") return json(response, 200, await capabilityProbe.start());
      if (request.method === "GET") return json(response, 200, await capabilityProbe.status());
      if (request.method === "DELETE") return json(response, 200, await capabilityProbe.remove());
