...
export async function startGodaddyServer({
  ...
  if (application !== undefined) {
    ...
    // persistent-store read, sidecar spawn or Matrix synchronization begins.
...
/**
 * Stops new HTTP accepts and Matrix intake immediately, lets active Settings
 * requests drain for a bounded interval, then tears providers/pool down last.
...
export async function shutdownGodaddyServer(server, {
  ...
  if (closeOutcome.completed) {
    // This call is idempotent: it observes the already-stopped Matrix service,
    // then closes provider processes and the shared MySQL pool in order.
    try {
