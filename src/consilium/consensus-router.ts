...
import type { SessionCloseObserver } from "./final-recommendation.ts";
...
export type ConsensusRouterInput = Readonly<{
  ...
  stopRuntimes?: () => void;
  beforeClose?: SessionCloseObserver;
}>;
...
export class ConsensusRouter {
  ...
  async run(input: Readonly<{
    ...
    const confirm = async (value: ConsensusConfirmation) => {
      ...
      if (value.kind === "final" || value.kind === "unresolved" || value.safetyHandoff === true) {
        const session = await registrar.getActiveSession();
        if (session?.generation !== input.sessionGeneration || session.phase !== "active") {
          throw new SafeConsiliumFailure("obsolete_generation");
        }
        try {
          await this.#input.beforeClose?.({
            session,
            messages: await registrar.getConfirmedMessages(input.sessionGeneration)
          });
        } catch {
          throw new SafeConsiliumFailure("archive_rejected");
        }
        const closed = await registrar.closeSession(input.sessionGeneration);
        if (!closed.ok) throw new SafeConsiliumFailure("archive_rejected");
      }
    ...
    const run = async (value: ConsensusDispatch, phase: ConsiliumPhase, assignment: string,
      ...
      await runtime.run({ phase, sessionGeneration: input.sessionGeneration, task: input.task, assignment, evidence,
        ...
        if (emission.body.trim().length === 0 || emission.body.length > 32_000 || containsSecretLikeContent(emission.body) ||
