...
const deliveryKey = (generation: number, sequence: number): string =>
  `registrar:delivery:${generation}:${sequence}`;
...
export type ConfirmedDeliveryRecord = Readonly<{
  generation: number;
  sequence: number;
  deliveryId: string;
...
export type ConfirmedMessageProjection = (
  storage: RegistrarStorage,
  record: ConfirmedDeliveryRecord
) => Promise<void>;
...
export type RegistrarOptions = Readonly<{
  storage: RegistrarStorage;
  now: () => Date;
  projectConfirmedMessage?: ConfirmedMessageProjection;
...
export class RegistrarDO {
  ...
  readonly #now: () => Date;
  readonly #projectConfirmedMessage: ConfirmedMessageProjection;
  ...
  constructor(options: RegistrarOptions) {
    ...
    this.#projectConfirmedMessage = options.projectConfirmedMessage ?? (async (storage, record) => {
      await storage.put(deliveryKey(record.generation, record.sequence), record);
    });
  ...
  async #appendConfirmedMessage(storage: RegistrarStorage, input: Readonly<{
    ...
    const deliveryRecord: ConfirmedDeliveryRecord = deepFreeze({
      generation: message.generation,
      sequence: message.sequence,
      deliveryId: deliveryIdFor(message),
      kind: "message",
      message,
    ...
    await this.#projectConfirmedMessage(storage, deliveryRecord);
  ...
  async stopSession(
    ...
    return this.#storage.transaction(async (storage) => {
      ...
      if (options.publishControl === true) {
        ...
        const controlRecord: ConfirmedDeliveryRecord = deepFreeze({
          generation,
          sequence: controlMessage.sequence,
          deliveryId: deliveryIdFor(controlMessage),
          kind: "control",
  ...
  async getConfirmedMessages(generation: number): Promise<readonly ConfirmedAgentMessage[]> {
    ...
    // The shared database pool also serves Settings and consultation requests.
   ...
   * Production may replace this with another atomic delivery projection.
   */
  async getLocalDeliveryRecordForTest(generation: number, sequence: number): Promise<ConfirmedDeliveryRecord | undefined> {
    return this.#storage.get<ConfirmedDeliveryRecord>(deliveryKey(generation, sequence));
  }
