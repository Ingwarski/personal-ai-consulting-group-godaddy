...
test("Critic message, count and canonical delivery commit exactly once", async () => {
  ...
  assert.equal((await f.registrar.getLocalDeliveryRecordForTest(1, result.value.sequence))?.message.body, confirmation.body);
...
test("delivery failure rolls back the count, message and proposal approval together", async () => {
  let failProjection = false;
  const f = await fixture(async (storage, record) => {
    if (failProjection) throw new Error("projection unavailable");
    await storage.put(`registrar:browser-delivery:${record.generation}:${record.sequence}`, record);
  });
...
test("all actors must explicitly agree to the same exact proposal; final is immutable and unique", async () => {
  ...
  assert.equal((await f.registrar.getActiveSession())?.phase, "active");
  assert.ok((await f.registrar.closeSession(1)).ok);
