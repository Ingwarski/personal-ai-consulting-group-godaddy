...
test("shared head assignment is a single idempotent record with all recipients bound to its fingerprint", async () => {
  ...
  const delivery = await registrar.getLocalDeliveryRecordForTest(1, 1);
  assert.equal(delivery?.message.internalEventId, input.messageId);
  assert.equal(await registrar.getLocalDeliveryRecordForTest(1, 2), undefined);
});
