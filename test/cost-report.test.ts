...
const factualInput = {
  ...
  infrastructure: [
    { service: "Hosting", period: "2026-08", amount: 0, currency: "USD", source: "Hosting invoice" },
    { service: "browser.org", period: "2026-08", amount: 0, currency: "USD", source: "current free plan" }
  ],
  providerUsage: [
...
test("reports only configured subscription fees, factual infrastructure spend and provider status without fabricating session token cost", () => {
  ...
  assert.match(body, /browser\.org: 0 USD за 2026-08/);
