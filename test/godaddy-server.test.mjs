...
import { FORBIDDEN_RUNTIME_ENVIRONMENT_NAMES as sharedForbiddenEnvironmentNames } from "../src/runtime/environment.ts";
...
test("runtime directories never reach HTTP handlers, including encoded names and authenticated requests", async t => {
  ...
  await withServer(t, { environment: supportedEnvironment, nodeVersion: "v22.16.0",
    ...
    for (const path of ["/runtime", "/runtime/private/program", "/runtime-release/private/manifest.json",
      "/%72untime/private/program", "/runtime%2fprivate/program", "/.runtime/private/program", "/.runtime-release/private/program"]) {
...
test("HTTP liveness remains available when asynchronous Browser application startup fails", async () => {
  let starts = 0;
  let stops = 0;
  let resolveStopped;
...
test("shutdown preserves MySQL until an active Settings request drains", async () => {
  const order = [];
  let releaseRequest;
  let requestStarted;
  ...
  const browserService = Object.freeze({
    stop: async () => { order.push("browser-stop"); }
  });
  const applicationRuntime = Object.freeze({
    browserService,
    settings: Object.freeze({
  ...
  await shutdown;
  assert.deepEqual(order, ["settings-start", "settings-end", "browser-stop", "providers-stop", "pool-end"]);
});
...
test("GoDaddy keeps the production credential denylist aligned with the shared runtime policy", () => {
  assert.deepEqual(godaddyForbiddenEnvironmentNames, sharedForbiddenEnvironmentNames);
});
