...
- The `0.6.6` musl binding imported on GoDaddy Node `v22.23.2` and opened then closed synthetic encrypted SQLite stores in both the fixed app and tmp probe directories. Its declared package engine is Node `>=24`.
- No Matrix account, room, credentials, user data or application database was used. The synthetic stores were removed immediately after the result.
- Published was not restarted, republished or otherwise changed.

## Result

`no_go: vendor_supported_matrix_e2ee_runtime_unavailable`.

GoDaddy Preview has usable writable storage across a restart, child processes and outbound Matrix HTTPS. The `0.4.0` release is blocked by a native ABI mismatch: its musl-named binary depends on the glibc loader that GoDaddy's musl runtime does not provide. The current `0.6.6` binary mechanically runs on this host, but the vendor explicitly supports it only on Node 24 or later while GoDaddy supplies Node 22.

That observation is not a production authorization: running a security-critical E2EE binding outside the vendor's declared Node range would breach the project's supported-runtime requirement. A viable path requires either a GoDaddy Node 24/glibc-compatible runtime or a vendor-supported musl native binding that supports Node 22. Shipping an unverified custom crypto binary would not be an acceptable security workaround.

## Cleanup

The `0.6.6` probe's `DELETE` request removed its two fixed SQLite-store directories after the result was read. The earlier probe's `DELETE` request had already removed its two fixed directories, and the diagnostic was read-only. This cleanup commit removes the temporary compatibility route, tests and native dependency from GitHub. It must be pulled into Preview only; it must not be published.
