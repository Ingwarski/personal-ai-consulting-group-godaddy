...
Чинна дельта SUR-01 за однойменними PRD, project-context, canonical-terms, guardrails, user-journey і screen-map. Штатні повідомлення Element, не новий web chat; SUR-02 і frozen файли не редагуються.

**Кожна агентська репліка SS-11:** рольова позначка + англійська назва автора → англійська назва адресата; HH:MM; далі повне дослівне повідомлення мовою сесії. Назва не ховається за кольором або emoji. Довгі назви переносяться; тіло, списки й таблиці не обрізаються. Нативна відповідь Matrix зберігає адресованість. Один бот лишається одним native sender.

**Доручення SS-10:** Head Consultant один раз називає мету й склад. Кожне окреме доручення містить конкретне питання, очікуваний результат, релевантні факти/обмеження та залежності. Однаковий загальний текст не розсилається як різні задачі.

**Уточнення SS-11:** Critic формулює проблему, її важливість і рекомендовану перевірку/зміну; спеціаліст відповідає саме на неї. Head Consultant публікує поточну спільну рекомендацію для погодження; підтвердження кожного стосується цього тексту. Технічні hashes, agent IDs, лічильник і provider dumps не показуються.

**Результат SS-20 / SS-19:** узгоджений фінал містить рішення й не більше трьох пріоритетних дій. За межі семи Critic-повідомлень кожному незгідному спеціалісту показати, що узгоджено, точну невирішену розбіжність і наступну перевірку; не ставити позначку консенсусу. Другий фінал для вже опублікованого результату не створюється.

**Мова та допомога:** уточнення SS-13 — мовою першого повідомлення, якщо її можна визначити; за невизначеності коротке питання про мову без удаваної впевненості. Назви ролей лишаються англійськими. SS-18/SS-19 пояснює доречну межу ШІ та потрібну згоду мовою сесії, без універсальної стіни disclaimer-тексту.

**Відновлення SS-12/SS-19/SS-27:** після повернення зв'язку назвати відому причину без секретів, збережений стан, автоматичну спробу та справді потрібну дію. Тимчасове відновлення не потребує Settings; пошкодження ключів/відкликання доступу має окрему безпечну інструкцію. Відправлення не видається за прочитання Власником. Stop залишається текстовою нативною командою.

Каркаси визначені, але нові мовні/рольові варіанти в Element ще не реалізовані й не перевірені. Нових browser receipts немає.
