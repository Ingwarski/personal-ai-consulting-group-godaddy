...
- Версія карти: V1
- Дата: 15.08.2026
- owner_invocation_id: `d960df76-734c-4d36-bf66-a6b2584b7763`

V1 не має окремих продуктових екранів. Єдина користувацька поверхня — одна приватна invite-only E2EE Matrix-кімната у штатному Element на Mac, iPhone, Samsung Flip7/Android і Windows PC. Ця карта визначає контракти груп повідомлень, стани й переходи всередині кімнати та нативні Element/Matrix-передумови першого приєднання, recovery, verification і revocation; вона не визначає макет, компоненти, фінальний текст, кастомний Element chrome або архітектуру.
...
- `docs/product-idea.md`: «Позиціонування», «Результат для користувача», «Основний сценарій», «Досвід живого чату», «Фінальна рекомендація», «Межі V1», «Правила даних і доступу», «Стандарт швидкості», «Критерій готовності V1» та «Підтверджені рішення».
- `docs/prd.md`: §3.2–3.4; §4–5; `US-001`–`US-023`; `FR-001`–`FR-036`; `NFR-001`–`NFR-015`; §8–12; `AC-001`–`AC-011`.
- `docs/project-context.md`: §6–14, зокрема Desired User Outcomes, Platform Targets, Core Scenarios, MVP Boundaries, Out of Scope, Constraints, Assumptions, Risks та Open Questions.
- `docs/canonical-terms.md`: `Власник`, `Запит`, `Сесія`, `Консиліум`, `Дослівний консиліум`, `Підтверджена репліка агента`, `Фінальна рекомендація`, `Технічна частина`, `Приватна Matrix-кімната`, `Дозволена Matrix-пара`, `Перевірений Matrix-пристрій`, `Hosted Matrix homeserver`, `Matrix E2EE`, `Recovery key`, `Згода на обробку даних`, `Особливо чутливий документ`, `Секрет`, `Архів сесій`, `Витрати сесії`; дії `Стоп`, `Нова задача`, `Витрати`, експорт і видалення; продуктові стани; «Screen / Flow Names»; Approved User-Facing Terms; Internal Terms; Open Vocabulary Questions.
- `docs/guardrails.md`: Source Of Truth Order, AI Autonomy Boundaries, Forbidden Changes, Scope Boundaries, Design Authority Rules, When To Ask, When To Stop, Artifact Separation Rules, Verification Rules, Evidence Requirements та Open Questions.
- `docs/user-journey.md`: Journey Overview; Journey Stages 1–10; Decision Points; Failure Path; Exit Points; Success State; Confirmed Facts And Constraints; Open Questions.
...
|---|---|---|---|---|---|
| `SUR-01` | Приватна Element/Matrix-кімната | Одна invite-only E2EE Matrix-кімната `Personal Consultant` у штатному Element на Mac, iPhone, Samsung Flip7/Android або Windows PC | Прийняти запит або точну команду, показати дозволи, перебіг роботи, результат, витрати й архівні дії в одному хронологічному потоці | Один Власник, один Matrix-акаунт бота на тому самому hosted homeserver, exact allowlist `room_id` + `owner_mxid`, перевірені й не відкликані пристрої, не більше однієї Активної сесії; без окремого браузерного UI, dashboard або іншого продуктового screen | `PI: Межі V1`; `PRD: FR-001, FR-034, NFR-005, NFR-007, NFR-013, §8, §11`; `PC: §7, §9–11`; `UJ: Starting Context, Stage 1, Confirmed Facts And Constraints` |

Нова репліка з'являється в `SUR-01`; продукт не створює окремої поверхні для її отримання.
...
| `MG-06` | Склад консиліуму | Показує конкретні предметні ролі 2–5 окремих Codex-агентів-спеціалістів і участь окремого Claude Code-агента-критика | Після маршрутизації складного, багатосферного, високоризикового або явно позначеного запиту в `Консиліум` | `PRD: FR-008–FR-010, AC-002`; `UJ: Stages 5, 6B` |
...
| `MG-09` | Витрати сесії | Показує фактичну вартість поточної сесії та сумарні витрати за поточний місяць; не створює непідтвердженого жорсткого ліміту | Після точної команди `Витрати` в активній або завершеній сесії з фактичними модельними викликами | `PRD: FR-033, AC-009`; `UJ: Stage 7` |
...
| `MG-11` | Збій або неповний результат | Називає фактичний збій, недоступного агента, неможливість скликати справжній консиліум, брак доказів, непідтверджену цілісність або безпечний room/device failure; пояснює наслідок і доступну наступну перевірку чи дію без розкриття protected data або секретних ключів | Коли повний успішний шлях недоступний і стан можна безпечно повідомити в `SUR-01`; інакше access failure завершується без захищеної відповіді | `PRD: FR-001, FR-023, FR-028–FR-030, NFR-007, NFR-012, AC-006–AC-007`; `GR: When To Stop, Evidence Requirements`; `UJ: Stage 1, Stage 8, Failure Path` |
...
| Потреба або гілка journey | Підтримка у `SUR-01` | Тип замикання | Трасування |
|---|---|---|---|
...
Surface closure досягнуто: кожен етап, decision point, failure path і команда з `docs/user-journey.md` має групу повідомлень, системну відповідь або явне пояснення, чому окремого screen немає.
...
flowchart LR
    ...
    G --> H{"Режим"}
    H -->|"Пряма відповідь"| I["MG-05: пряма відповідь"]
    H -->|"Консиліум"| J["MG-06: склад консиліуму"]
...
flowchart LR
    ...
    C --> A
    B -->|"Витрати"| D["MG-09: Витрати сесії"]
    D --> A
...
- Єдина продуктова локація V1 — `SUR-01`; URL, route path, вкладок, бокового меню, окремої архівної сторінки, cost-dashboard або consent-center немає. Native Element/Matrix invitation, recovery, verification і revocation є host-client передумовами, а не додатковими продуктовими screens.
...
- Усі нові репліки залишаються в тій самій E2EE Matrix-кімнаті; продукт не додає іншої поверхні, окремих Matrix-акаунтів агентів або власного звуку.
...
| 6A. Пряма відповідь | `MG-05` | Простий запит | Завершення й архів за підтвердженої цілісності | `FR-028–FR-030`, `AC-001` |
| 6B. Склад і старт консиліуму | `MG-06`, потім `MG-07` | Складний або явно позначений запит | До живого перебігу; перша агентська репліка до 30 секунд | `FR-009–FR-015`, `NFR-002`, `AC-002` |
...
| 10. Експорт | `MG-13` | Запит Власника | Повний експорт; повернення до незміненого архівного стану | `FR-032`, `AC-008` |
...
| `SS-02` | Очікування Згоди на обробку даних | `MG-02` просить одноразову згоду через оператора hosted Matrix homeserver, Cloudflare, OpenAI та Anthropic і чесно пояснює metadata/plaintext trust boundary | Згода → `SS-03`; відмова/відсутність → звичайна обробка не починається | `FR-002`, `NFR-005`; `UJ: Stage 2` |
...
| `SS-08` | Активна сесія: запит прийнято | `MG-04` показує підтвердження до 5 секунд і, для нової сесії, дату й час старту | `SS-09` або `SS-10`; уточнення й команди доступні | `FR-007`, `FR-017`, `NFR-001` |
...
| `SS-14` | Витрати показано | `MG-09` містить фактичну вартість поточної сесії та суму за місяць без жорсткого ліміту | Повернення до попереднього active/completed context | `FR-033`, `AC-009` |
...
| `SS-27` | Room invariants не підтверджено | E2EE/invite-only, joined membership рівно Власник + бот на одному hosted homeserver, no pending invites, history `joined`, no guests/bridges/widgets і federation gate не підтверджено | Після відновлення інваріантів → `SS-01`; без цього exit. Непідтримуваний `m.federate: false` потребує security exception Власника до production | `GR: Forbidden Changes, When To Ask, When To Stop`; `UJ: Stage 1` |

### Обов'язкові загальні state-категорії

| Категорія | Контракт для `SUR-01` |
|---|---|
...
| Same-chat delivery | Кожна нова вихідна репліка з'являється в `SUR-01`; окрема продуктова поверхня не створюється. |
...
8. `Витрати` є переривним read-only маршрутом: після відповіді активна робота може продовжитися; команда не встановлює ліміт.
9. Під час консиліуму `MG-07` і `MG-08` утворюють видимий progress-loop. Якщо агент не демонструє поступу, його завдання звужується або агент замінюється з правдивим повідомленням.
10. До спливу 10 хвилин без готової Фінальної рекомендації маршрут переходить у `SS-17`. Явний дозвіл повертає до progress-loop; без дозволу робота не продовжується.
11. Окремий дозвіл стосується лише названої межі. Згода на обробку даних не дозволяє чутливий документ чи зовнішню дію; мовчання не є дозволом.
12. Якщо доказів недостатньо або потрібна зовнішня перевірка, маршрут переходить у `SS-19`, а не до впевненої рекомендації.
13. Перехід у `SS-21` дозволений лише коли повноту, порядок, відсутність дублів і незмінність запису підтверджено. Інакше показується частковий статус і наступна перевірка.
14. Видалення окремої репліки не має переходу до success-state. Видаляється лише ціла сесія після повторного явного підтвердження.
15. Нова репліка належить лише `SUR-01` і не створює іншої продуктової локації. Продукт не створює власного звуку.
...
| Entry | `Витрати` | Команда в активній або завершеній сесії з фактичними модельними викликами | `MG-09`, потім повернення до поточного контексту |
...
7. Який оператор hosted Matrix homeserver використовується та чи підтримує він `m.federate: false` під час створення кімнати? Це не додає screen: без підтримки потрібні зафіксований security exception і явне рішення Власника до production.

Жодне відкрите питання не дозволяє додати іншу користувацьку поверхню, паралельну сесію, новий тип вкладення або автоматичну зовнішню дію до V1.
