...
**Outcome:** with Settings closed, a real Published Owner message reaches Head Consultant, assigned specialists, the selected Critic and one delivered result; the same identity and accepted work survive restart/redeployment without duplicates. Missing old keys remain a separate recovery blocker, not a reason to reset identity.

**Dependencies and ownership:** existing U-01 configuration, U-05 registrar/dedupe/outbox and U-06 Rust/supervisor seams; independent provider routing remains unchanged. Rust owns the direct SDK MySQL adapter and migration reader; Node owns explicit child configuration, supervisor, canonical ACK/outbox and connection UI. Interface producer/consumer: Rust emits the existing bounded `ValidatedMatrixIngress`; Node commits and returns existing ACK; Rust sends the existing deterministic outbox transaction ID. Preserve protocol compatibility unless a tested versioned change is necessary. No DB bridge, extra service or paid/local production host.

| Milestone | Work and proof before advancing |
|---|---|
| 1. Adapter contract | Inventory pinned SDK crypto/state traits and all restart-critical application files. Implement AD-26 additive schema and encrypted direct MySQL adapter with reused DB/store secrets, parameterized/TLS transport, atomic batches and DB fencing. Disposable caches stay in memory. Run SDK store-contract and isolated real-MySQL positive/negative tests; synthetic cipher tests alone do not pass. |
| 2. Recovery and migration | Preserve committed cursor, durable pending events/ACK linkage and pending media recovery. Inject termination/DB disconnection/unknown commit/stale-writer faults. Build pinned SQLite-schema-aware import, verify exact identity/counts/decryption/replay and retry after interruption. Prove isolated restore and reject stale SQLite rollback after MySQL writes. No usable old store/key means no production migration, not empty-store provisioning. |
| 3. Runtime and setup | Wire backend to existing Rust/Node host, package compatible immutable binary, verify explicit child env. Remove public-folder/timed Preview ceremony only after private durable-file inventory is empty. Keep private transient media safeguards, current owner login, genuine device verification, existing usable controls and stateless Preview. Reuse startup/reconnect/same-app wake; no Settings dependency. |
| 4. Published acceptance | After authorized additive provisioning/cutover, verify same deployed revision/schema/device: Settings closed → message → consultation → Critic → one result, then restart, redeploy and temporary DB interruption. Inspect actual received response and duplicate/accepted-event lineage. Do not close the unit on a build, push, pusher registration or HTTP health alone. |

**Verification:** `QA-INT-019`–`QA-INT-024`, `QA-SEC-001`, `QA-REG-010`, `QA-CNS-007`, `QA-CNS-008`, `QA-CNS-009`, `QA-CNS-010`; gates G-14, G-15, G-23 and existing applicable usability/visual/user-task gates. New target definitions are prepared, runs not_run. Existing commands such as `cargo test --locked` and `npm run check` provide only their exercised local scope; the implementation must add and record the real-MySQL test harness rather than invent an existing command. Evidence binds actual revision, binary, schema, environment and test case; never log credentials or private messages.

**Design/UX:** retain baseline `PC-MATRIX-CANDIDATE-B-V2-20260816-R1`, target `07e3675265e8cadef1e65c132f32e3cbbf4d6537cbfd316ea16a6bacd56f1bd6`, native Element SUR-01 and existing SUR-02 utility hierarchy. No prototype promotion/reuse starts here. QA-CNS-010 covers H1–H10 recovery/status clarity; QA-CNS-009 remains an actual Owner task, not simulated user evidence. Removing obsolete hosting-folder steps does not remove authentication or actual Matrix trust verification.

**Operations and rollback:** runtime owner handles content-free DB/fence/sync alerts; migrations are additive and explicit, existing unrelated tables are untouched. Before cutover retain verified legacy state; after new MySQL writes only compatible code rollback or a separately verified reverse migration is safe. Future dependency/key maintenance follows AD-26 without adding an MVP key-rotation UI. No wipe, legacy resource cleanup, silent replacement device, MFA, hosting/domain migration or HappyPro change.

**Handoff status:** formal correction plan prepared; implementation and Published verification not performed by this SDD pass. The latest “implement it, plz” preceded this formal reconciliation. Under the installed pipeline contract, preserve that instruction as source adoption, invalidate old plan authorization, and pause for a later explicit implementation message after this plan validates. Do not manufacture its timestamp or release the gate retrospectively.
