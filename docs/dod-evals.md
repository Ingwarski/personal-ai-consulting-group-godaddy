...
- Дата: 06.09.2026
- owner_invocation_id: `dod-evals-approved-critic-router-20260906`
...
| `docs/design-brief.md` | `a5feac6981acefb15cc84e827080d72b4b7b3734a1e6487cf4046f611095396e` | Approved `PC-MATRIX-CANDIDATE-B-V2-20260816-R1`; `DB-D18`/`DB-D19`/`DB-D20`; both surfaces; `P-01`–`P-15`; H1–H10; representative task plan; HappyPro palette for `SUR-02` |
...
- one fenced Codex subscription process/credential lineage → distinct head/specialist thread IDs and isolated workspaces without cloned `auth.json`/OAuth caches; selected separate critic runtime → subscription setup-token/auth-status path;
...
System/end-to-end evidence запускає `AC-001`–`AC-016` через реальну приватну E2EE Matrix-кімнату та реальну protected `SUR-02`: точні room/mxid, перевірені Element/bot devices, production Rust `matrix-sdk` sidecar з exact encrypted store/device binding, bounded private NDJSON, deployed GoDaddy Node 22 server side, Google OIDC і перевірка окремої локальної підписаної сесії, MySQL registrar/outbox/settings/encrypted archive, one fenced Codex OAuth lineage/writer, distinct real Codex threads/workspaces, real separate selected subscription-OAuth critic, configured A2A adapter and actual cost records.
...
- `G-17` підтверджує актуальну private single-owner plan eligibility, subscription OAuth-only mode, один authoritative Codex writer/checkpoint lineage, окремі реальні agent contexts, Claude setup-token process лише для Claude-маршруту, quota/fail-closed behavior і відсутність paid fallback;
...
- **Required Evidence:** integration lease/start/heartbeat/end records for one head and 2–5 separate Codex thread/workspace identities inside one account runtime, plus one separate selected-critic context (Claude Code process/workspace or Codex thread/workspace); bounded objectives; independent first-pass evidence; E2E visible concrete roles, critic contribution, addressed discussion and truthful unavailable/replacement path. OAuth values are excluded from evidence.
- **Pass Condition:** simple request avoids needless roster; every visible Codex role maps to a distinct current thread/workspace/lease, critic maps to a distinct context of its selected provider, and full Консиліум has the required roster and independent-first-pass behavior. A shared OAuth account does not collapse agent identity, and separate agents do not require cloned credential stores.
...
- **Historical VisualQAEvidence References:** `forge/design/evidence/candidate-b/v2/visual-qa.json` and its four listed screenshots prove the approved frozen candidate only; because they retain superseded auth-copy, fresh implementation evidence must demonstrate `DB-D18`/`DB-D19`/`DB-D20`.
...
- **Source References:** PRD `FR-037`, `FR-040`–`FR-045`, `NFR-018`, `AC-016`; wireframes `SUR-02`, `SG-01`–`SG-05`, `SS-30`–`SS-46`; design brief `DB-D18`/`DB-D19`/`DB-D20`, `P-11`–`P-15`, Accessibility Floor, Responsive And Platform Behavior; architecture §§2–4, 8, 12; guardrails `Verification Rules`, `Evidence Requirements`.
...
- **Rerun Rule:** after affected journey, route, state, copy, control, error/recovery, accessibility behavior, supported client/viewport, design baseline or `DB-D18`/`DB-D19`/`DB-D20` change; rerun affected H1–H10 scopes and any combined-impact regression set.
...
Підстава: `PI-CRITIC-20260906`, `FR-010/040–046`, PRD security delta, `AD-24`, `DB-D20`. Визначення гейтів узгоджені з тим самим frozen baseline/target; нових гейтів немає. **Definition Status: prepared; Execution Status: not_run; Release Readiness: not_evaluated.** Цей owner не запускав тести.

| Чинний гейт | Додатковий параметр, потрібний доказ і умова проходження |
|---|---|
| `G-05 real_consilium_runtime` | Окремі runs Claude/Codex із distinct designated critic context. Codex-критик — fresh thread, не head/fork; застосовані саме його model/effort. Потрібний реальний production caller та повний critique→revision→final flow; тестовий launcher або каталог самі не є E2E. |
| `G-06 canonical_a2a_verbatim_order`, `G-09 truthful_result_and_action_contract` | Review receipt зв'язує designated agent/provider/runtime/event/generation; head/specialist/інший runtime/старий receipt не відкривають final gate. Повні репліки, canonical order і generation fencing збережені. |
| `G-17 subscription_oauth_private_runtime` | Обов'язкові auth/eligibility/quota лише Codex і вибраного Критика. Codex-маршрут має no-Claude-call evidence за absent/expired/403/error/hung Claude; Claude-маршрут окремо доводить готовність Claude. Старі Claude-specific cases застосовні саме до Claude-маршруту, не блокують Codex. No fallback, no cloned OAuth, no secret leaks. |
| `G-19 owner_settings_atomic_configuration` | Структура трьох груп включає explicit Critic provider і незалежні branch preferences. Потрібні current host Astra/xhigh confirmation, hidden-catalog handling, current/default separation, migration v1/v2/v3 і historical snapshots/ledgers, CAS/idempotency/restart/rollback evidence. Неактивні capabilities не є eligibility gate; їх не виконують. Reset недоступного default не підміняється. |
| `G-20 owner_settings_runtime_ux`, `G-16 approved_visual_baseline_fidelity` | Selector і лише активні critic fields доступні без Claude-каталогу, у чинному P-12 layout/палітрі з DB-D20. Keyboard/focus/status/reflow, збереження уподобань і незмінність head/snapshot потребують окремого browser/runtime evidence. |
| `G-21 heuristic_usability_review`, `G-22 representative_user_task_validation` | H1–H10 для перемикання та відновлення; UV-04/чинний owner task після реалізації. Не підміняти observed owner completion агентською перевіркою. |
| `G-23 product_security_requirements` | Додатково NFR-006/007/008/009/010/017/019 за AD-24: designated identity, unchanged authority, no inactive-provider data/calls, strict versioned config і atomic migration. Решта одинадцяти вимог лишається чинною; нових винятків немає. |
| `G-15 v1_real_matrix_e2e` | Повний live route від owner Settings та Matrix ingress до archive/final на тому самому deployment lineage, з окремим результатом для кожного провайдера. Expired Claude не може бути позначений live-pass. |

Будь-яка заміна провайдера/моделі/effort без явного вибору, stale identity receipt, втрачена історія, спільний critic/head context, прихована залежність Codex від Claude або незафіксований production caller блокує зачеплений гейт. Повтор потрібний після router/schema/catalog/runtime/auth/receipt/UI/deployment зміни. Старі Claude-only results не перевіряють новий Codex-маршрут; історичні результати не видаляються й не перепозначаються як нові. QA owner прив'яже конкретні існуючі/нові checks; ці визначення не стверджують, що їх виконано.
