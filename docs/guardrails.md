...
- `docs/canonical-terms.md`: `Власник`, агентські ролі, `Консиліум`, `Дослівний консиліум`, Matrix/data/archive terms, `Облік витрат`, `Налаштування власника`, `Google-вхід`, `Codex-агенти`, `Claude Code-критик`, provider-specific `Міркування`, `Швидкість консиліуму`, `Пресет швидкості`, `Фактичні налаштування сесії`, `Subscription OAuth`, `Codex OAuth-стан`, `Claude Code OAuth token`, `Fail-closed auth/quota`, `A2A`, `Реєстратор`, `Канонічний порядок`, `Незмінний журнал сесії`, `allowlist`, `room_id` і `owner_mxid`, а також розділи 6–11.
...
- Не замінювати підтверджені платформи й межі: Element/Matrix на публічному `matrix.org`, Cloudflare, OpenAI/Codex, Anthropic/Claude Code та A2A. WhatsApp і Meta WhatsApp Cloud API є superseded та out of scope.
...
- Не стверджувати, що Matrix E2EE приховує службові метадані від `matrix.org` або захищає розшифрований зміст після завершення E2EE на verified Matrix device бота в Cloudflare runtime. OpenAI та Anthropic отримують переданий їм зміст через TLS.
...
- `Облік витрат` охоплює налаштовані місячні платежі ChatGPT/Codex і Claude, фактичні витрати Cloudflare, R2 та іншої інфраструктури, а також поточний статус `matrix.org` (`0` лише доки доступний його безкоштовний план) і доступний provider usage/limit/reset; для підписного AI-usage не створюється per-session charge.
...
- Перед першою звичайною обробкою бізнес-даних — одноразову `Згоду на обробку даних` через `matrix.org`, Cloudflare, OpenAI та Anthropic із чесним поясненням межі Matrix E2EE.
...
- Перевірка довірчої межі має окремо довести шифрування подій між перевіреними Element/Matrix-пристроями й зафіксувати, що `matrix.org` бачить метадані, а розшифрований у Cloudflare runtime зміст передається OpenAI та Anthropic через TLS. Не подавати це як E2EE до AI-провайдерів.
...
- Скріншот, статична документація, mockup, prototype, згенерований HTML, синтетична Matrix-подія, попередній WhatsApp prototype або локальний browser-preview окремо доводять лише відповідну статичну, історичну чи локальну властивість. Вони не доводять готовність Matrix E2EE, `matrix.org`, Cloudflare, A2A, агентського циклу, архіву або повного V1.
