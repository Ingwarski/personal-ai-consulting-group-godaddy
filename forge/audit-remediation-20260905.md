...
Status: in progress. This is an engineering evidence report, not a release approval or a replacement development plan.
...
| Tests ran on a different Node version and Linux packaging was only planned | Run the complete suite on official, checksum-verified Node 22.16.0; add credential-free Linux Node 22 and static musl tests to the non-deployable workflow | Local Node suite: 360/360, TypeScript/build and environment policy pass. Linux CI has not yet run; a prepared workflow is not execution evidence |
