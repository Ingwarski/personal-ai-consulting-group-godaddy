// Operator verification for this exact release; never runs the downloaded programs.
...
const commit = "271bcb8ec973e34fe0e53a0f260991ed1a4a5373";
const runId = 34060973791;
const artifactId = 9998728398;
const archiveDigest = "676a5d7a850dcead677580f45b7bae222e4e224d24d1ae22c2795f0e7b8eb891";
...
const scratch = mkdtempSync(join(tmpdir(), "matrix-release-verified-"));
...
assert.deepEqual(manifest.artifacts.map(({ path }) => path).sort(), expectedNames.filter((name) => !["release-manifest.json", "SHA256SUMS"].includes(name)));
for (const entry of manifest.artifacts) {
  ...
  } else assert.equal(entry.role, "evidence");
}
const sums = files.get("SHA256SUMS").toString("utf8").trim().split("\n");
assert.equal(sums.length, files.size - 1);
assert.deepEqual(sums.toSorted(), [...files].filter(([name]) => name !== "SHA256SUMS").map(([name, bytes]) => `${sha256(bytes)}  ${name}`).sort());
...
assert.deepEqual(provenance.builder, { ...builder, observedImageConfigDigest: builder.imageConfigDigest });
for (const key of ["image", "imageConfigDigest", "platform", "rustToolchain", "target", "systemHeaders"]) assert.deepEqual(manifest.builder[key], builder[key]);
...
for (const key of ["binaryByteEquality", "staticStrippedElfCheck", "credentialFreeMuslTests"]) assert.equal(provenance.build[key], "passed");
...
const sourcePaths = git("ls-tree", "-r", "--name-only", commit, "--", ...matrixReleaseSourceMounts.map(({ source }) => source),
  ...
  "scripts/matrix-release-package.mjs", "scripts/build-matrix-release.mjs", "test/matrix-release-package.test.mjs").toString().trim().split("\n").sort();
...
assert.deepEqual(provenance.subjects.toSorted(byRole), manifest.artifacts.filter(({ role }) => ["sidecar", "setup"].includes(role)).sort(byRole));
const receipt = { verifiedAt: new Date().toISOString(), runId, runAttempt: 1, sourceCommit: commit,
  artifactId, archiveSha256: archiveDigest, archiveSizeBytes: archive.length, fileCount: files.size,
  manifestSha256: sha256(manifestBytes), verifiedSourceInputCount: sourcePaths.length,
  binaries: provenance.subjects, result: "passed", limitation: "Source-bound GitHub build evidence; not real-host execution or Matrix authentication proof." };
if (process.argv.includes("--stage")) {
  const destination = join(root, ".runtime-release", "matrix");
  assert(!existsSync(destination), "Release bundle already exists; refusing overwrite");
  mkdirSync(destination, { recursive: true, mode: 0o755 });
  for (const [name, bytes] of files) writeFileSync(join(destination, name), bytes, { flag: "wx", mode: 0o644 });
}
console.log(JSON.stringify({ ...receipt, downloadedArchive: zip }, null, 2));
